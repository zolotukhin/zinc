...
```mermaid
flowchart LR
    A[GGUF models] --> B[Vulkan kernels tuned for RDNA3 and RDNA4]
    B --> C[Continuous batching and paged KV cache]
    C --> D[TurboQuant KV compression]
    D --> E[OpenAI-compatible local inference server]
    E --> F[Useful local AI on AMD consumer GPUs]
```

Caption: ZINC is not just a kernel experiment. It is a full stack designed to turn AMD consumer GPUs into practical local AI servers.
