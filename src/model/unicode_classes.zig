...
pub const number_ranges = [_][2]u32{
    .{ 0xB2, 0xB3 },       .{ 0xB9, 0xB9 },       .{ 0xBC, 0xBE },       .{ 0x660, 0x669 },
...
pub const space_ranges = [_][2]u32{
    .{ 0x85, 0x85 },     .{ 0xA0, 0xA0 },     .{ 0x1680, 0x1680 }, .{ 0x2000, 0x200A },
    .{ 0x2028, 0x2029 }, .{ 0x202F, 0x202F }, .{ 0x205F, 0x205F }, .{ 0x3000, 0x3000 },
};

fn inRanges(table: []const [2]u32, cp: u32) bool {
    var lo: usize = 0;
...
/// Returns true when a non-ASCII code point is a letter (\p{L}).
/// @param cp Unicode code point, expected >= 0x80.
/// @returns Whether the code point is in a letter range.
pub fn isLetter(cp: u32) bool {
    return inRanges(&letter_ranges, cp);
}

/// Returns true when a non-ASCII code point is a combining mark (\p{M}).
/// @param cp Unicode code point, expected >= 0x80.
/// @returns Whether the code point is in a mark range.
pub fn isMark(cp: u32) bool {
    return inRanges(&mark_ranges, cp);
}

/// Returns true when a non-ASCII code point is a number (\p{N}).
/// @param cp Unicode code point, expected >= 0x80.
/// @returns Whether the code point is in a number range.
pub fn isNumber(cp: u32) bool {
    return inRanges(&number_ranges, cp);
}

/// Returns true when a non-ASCII code point is whitespace (\s).
/// @param cp Unicode code point, expected >= 0x80.
/// @returns Whether the code point is in the whitespace set.
pub fn isSpace(cp: u32) bool {
    return inRanges(&space_ranges, cp);
}

test "unicode class tables classify common scripts" {
    const std = @import("std");
    try std.testing.expect(isLetter(0x4E2D)); // CJK ideograph
    try std.testing.expect(isLetter(0x00E9)); // e with acute
    try std.testing.expect(!isLetter(0x3002)); // ideographic full stop
    try std.testing.expect(isNumber(0xFF11)); // fullwidth digit one
    try std.testing.expect(isMark(0x0301)); // combining acute accent
    try std.testing.expect(isSpace(0x3000)); // ideographic space
    try std.testing.expect(!isSpace(0x4E2D));
}
