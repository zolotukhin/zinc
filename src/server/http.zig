...
pub const Connection = struct {
    ...
    /// Read and parse an incoming HTTP request from the connection stream.
    /// @returns A `Request` with method, path, and body extracted from the raw HTTP message.
    ...
    /// Write an HTTP response with status, content type, and body to the connection stream.
    ...
    /// Close the connection stream and release the underlying socket.
...
pub const Server = struct {
    ...
    /// Stop listening and release server resources.
