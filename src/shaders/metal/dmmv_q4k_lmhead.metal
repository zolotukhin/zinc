...
// Large-M specialization for the LM head: more rows per threadgroup than the
// generic kernel so the staged input vector is reused across a wider row slice.
// The current Qwen3.5 Metal path uses K=2048 here, so 512 threads map cleanly
// to the 512 float4 chunks in the input vector.
#define TG_SIZE 512
