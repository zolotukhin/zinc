...
// Dedicated LM-head specialization for K <= 2048 on Apple GPUs with 1024-thread
// threadgroups. The vocab projection is the largest decode-side matvec by row
// count, so widening to 32 rows per threadgroup halves the number of launches
// relative to the 512-thread path while keeping the staged 8 KiB input cache.
#define TG_SIZE 1024
