...
kernel void main0(
    ...
    device const float* weights0 [[buffer(3)]],
    device float* output1 [[buffer(4)]],
    device const float* weights1 [[buffer(5)]],
    ...
    for (uint i = tid; i < p.n; i += tg_size) {
        ...
        output0[base + i] = weights0[p.weight0_offset + i] * v;
        output1[base + i] = weights1[p.weight1_offset + i] * v;
    }
}
