...
kernel void main0(
    ...
    for (uint i = tid; i < p.n_experts; i += 64u) {
        const float v = logits[i] * logit_scale;
        probs[i] = v;
    ...
    for (uint ki = 0u; ki < p.k; ki++) {
        ...
        if (tid == 0u) {
            ...
            output_data[p.k + ki] = as_type<uint>(global_best);
    ...
    if (tid == 0u) {
        ...
        for (uint i = 0u; i < p.k; i++) {
            wsum += as_type<float>(output_data[p.k + i]);
        }
        const float inv_wsum = (wsum > 0.0f) ? (1.0f / wsum) : 0.0f;
        for (uint i = 0u; i < p.k; i++) {
            const float w = as_type<float>(output_data[p.k + i]) * inv_wsum;
            output_data[p.k + i] = as_type<uint>(w);
        }
    }
}
