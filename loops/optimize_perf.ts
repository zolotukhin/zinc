...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  11: {
    ...
    knownFlatCategories: [
      "LDS STAGING OF K OR V IN flash_attn IS UNAMBIGUOUSLY DEAD. Three attempts have lost: K-parallel-with-subgroupAdd (-44% in run-1 cycle 1), V-tile chunk=16 (-12% in run-1 cycle 2), K-row chunk=32 (-29% in run-2 cycle 6). The 8-cycle vec4 LDS read floor combined with 4-way bank conflict multiplier (~32 cycle reads) exceeds what 16-way ILP hides on cached scattered global K/V reads (~3-cycle effective). Plus +16 KB LDS halves WGs/CU occupancy. DO NOT propose any cooperative LDS staging for K or V in any form on flash_attn.",
      "GQA COLLAPSE Q_PER_KV=4 KILLS CONCURRENCY. Run-2 cycle 3 implemented full collapse (4 Q heads per WG, dispatch 32→8 WGs) and measured -9.9% at L=1500. R9700's 64 CUs need many WGs in flight to hide vmem latency; 8 WGs starves the SIMD pool. The K/V amortization is real but the WG-count loss dominates. The 273% effective bandwidth utilization shows L2 is already capturing inter-Q-head reuse passively. Don't re-attempt with q_per_kv=4. If GQA collapse is re-attempted at all, only with q_per_kv=2 (preserves 16 WGs) OR split-K dispatch (n_kv_heads × n_i_chunks WGs) — see structuralSwingIdeas.",
      "PHASE 4 MICRO-CHANGES ARE FLAT. V-load promotion above score multiplies (run-2 cycle 1, flat); drop redundant post-Phase-4 barrier alone (run-2 cycle 4, flat); subgroupShuffleXor merge + drop barriers consolidation (run-1 cycle 13 flat, run-2 cycle 5 flat); register-resident accumulators across blocks (run-1 cycle 15, flat); 32-way ILP (run-1 cycle 18, flat). The 16-way ILP shape is the saturation point. Don't propose further Phase 4 micro-tuning.",
      "BLOCK_SIZE TUNING IS FLAT. 256→512 (run-2 cycle 2, flat); 256→384 (run-2 cycle 13, +0.34 below threshold). Don't re-attempt BLOCK_SIZE changes on flash_attn.",
      "K+V PROJECTION FUSION REGRESSES. Dedicated dmmv_q4k_fused_kv shader (run-2 cycle 10, -1.1%). The two projections already overlap on RDNA4; fusing adds register pressure without saving wall time. Don't re-attempt.",
      "GATE+UP-ONLY FUSION (without SwiGLU) IS FLAT. Run-2 cycle 7: ON=OFF=93.67. The fusion only wins WITH SwiGLU folded in (run-2 cycle 8 fuses all three for +1.32). Don't re-test gate+up alone.",
      "ffn_norm + gate+up+SwiGLU 4-WAY FUSION IS BELOW THRESHOLD. Run-2 cycle 9 added RMS norm into the cycle-8 fusion: +0.16 tok/s, well below the +0.94 keep threshold. The 3-way fusion is already taking the bulk savings; adding RMS norm doesn't add measurable wall time savings.",
      "PER-TOKEN COS/SIN PRECOMPUTE FOR RoPE IS BELOW THRESHOLD. Run-2 cycle 13: +0.29 tok/s. The pow+cos+sin per RoPE WG is small; precomputing once per token doesn't move the needle.",
      "CYCLE-8 FUSION VARIANTS (loop swap, NUM_ROWS=4) ARE FLAT. Run-2 cycle 11. The current shape of dmmv_q4k_fused_dense_ffn is the right answer; don't tune it further.",
      "MANUAL-CYCLE FOUNDATIONS THAT LANDED — DON'T REVERT: Q-stage in flash_attn (commit 6ece0a8); per-block page_ids[] cache superseded by s_kv_base_v4 (commits f68b7d7 + run-1 cycle 8); Phase 4 rescale+V-acc fusion (commit 539b2aa). All bit-correct across 30+ cycles of build-on at L=1500.",
      "L=2325 GPU HANG IS L≥2300-ONLY. Confirmed across 30+ cycles measuring at L=1500 with no hangs. Most likely a watchdog-duration issue from very long flash_attn dispatches at high context, not a correctness bug. Defer to a separate effort if L>2300 needs fixing.",
    ],
    structuralSwingIdeas: [
      "CROSS-LAYER FUSION: O-PROJ-RESIDUAL + NEXT-LAYER ATTN-NORM. Run-2 cycle 12's win was within-layer (Q+K norm+rope+kv_write fused). The next analog is across the layer boundary: fold the output projection's residual-add into the next layer's RMS norm via a partial-sum buffer pattern. Saves 1 dispatch + 1 barrier per attention layer = 36/token at L=1500. Reference: cycle 12's wg_id-branched dispatch (single shader, branches on workgroup ID to handle different sub-tasks). The agent's cycle-12 nextIdeas list explicitly flagged this as the next attack.",
      "CROSS-LAYER FUSION: DOWN-PROJ-RESIDUAL + NEXT-LAYER ATTN-NORM. Mirror of the previous on the FFN→attention boundary. Same shape, same expected ~+1-2 tok/s win, same partial-sum buffer pattern. Reference: dmmv_q4k_fused_dense_ffn for the FFN-side dispatch shape that already produces down-residual; the new fusion would extend it to also write the next layer's pre-norm hidden state.",
      "GQA COLLAPSE Q_PER_KV=2 (preserves concurrency). Half the K/V amortization but doubles WGs vs Q_PER_KV=4: dispatch n_kv_heads*2 = 16 WGs (vs 8 for full collapse, 32 for none). 16 WGs on R9700's 64 CUs keeps concurrency above the latency-hiding floor while still amortizing K/V tile reads 2-fold. Re-implement run-2 cycle 3's flash_attn_gqa.comp with Q_PER_KV=2 + per-WG-handles-2-Q-heads layout. LDS budget: ~6 KB per WG (vs 11.5 KB at q_per_kv=4) — strictly better.",
      "GQA COLLAPSE WITH SPLIT-K DISPATCH. Restore concurrency lost by full GQA collapse via i-axis splitting: dispatch n_kv_heads × n_i_chunks WGs (e.g., 8 × 4 = 32 WGs to match the no-collapse baseline) where each WG handles q_per_kv Q heads on its assigned i-chunk of the KV cache. Requires a merge pass to combine partial Of[r], Lf[r], Mf[r] across i-chunks per (head, kv_head). More complex implementation; multi-cycle. Reference: llama.cpp flash_attn split-K reduce shader.",
      "V-CACHE FLOAT16 QUANTIZATION. Halves V bandwidth (the dominant ~56% slice of decode time per the cycle-17 phase budget) and doubles L2 effective capacity. kv_cache_write side change: write V as f16 instead of f32; flash_attn V-acc inner loop reads f16 and unpacks to f32 for FMA. Tolerance: f16 V at decode time has tested cleanly in many production engines (llama.cpp's `-ctv f16` is the default). Multi-cycle: requires two-shader change (kv_cache_write + flash_attn) plus a config knob. Validate against per-token reference at tol=1e-3.",
      "INSPECT THE FLASH_ATTN PER-LAYER OUTLIER PATTERN AT L=1500 USING THE CYCLE-13 ZINC_FA_PROFILE_LAYER FLAG (kept in run-2 cycle 13 enablement). Cycle 13 of run-2 confirmed all 36 layers run uniformly ~0.014ms at L=1, but the L=1500 distribution is unmeasured. If a specific layer or specific layer-class (full-attn vs SWA, first vs last) drags the bucket, that informs where to attack next. Cheap diagnostic.",
      "flash_attn_cm1.comp KHR COOPMAT PORT. RDNA3+ supports VK_KHR_cooperative_matrix and R9700 advertises it. Cooperative-matrix wmma intrinsics on the Q.K matmul and the softmax-V matmul could deliver substantial throughput on attention if the layout matches. Multi-week port; the structural ceiling for any further attention speedup likely lives here. Reference: /Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/flash_attn_cm1.comp. Pursue when cross-layer fusions and GQA-q2/split-K are exhausted.",
    ],
    referenceImplementations: [
      {
