...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  11: {
    ...
    knownFlatCategories: [
      "FFN SUB-BUCKET CALIBRATION (run-4 cycle 12): gate+up+SwiGLU is 60% of FFN bucket (3.55 ms = 35% of total decode at L≈846). down_proj is only 40% of FFN (2.40 ms = 24% of total decode). The 'attack down_proj first' premise from prior swing lists was WRONG — gate+up+SwiGLU is the dominant sub-bucket. Direct attacks on it are listed in this flat-list (algebraic, paired acc, loop swap, uvec4 alias all failed) so the next attempt must be a different structural shape (split-K on K=4096 axis is the leading candidate, see structuralSwingIdeas).",
      "LDS STAGING OF K OR V IN flash_attn IS UNAMBIGUOUSLY DEAD. Three attempts lost (-44%, -12%, -29%). The 8-cycle vec4 LDS read floor + bank-conflict multiplier exceeds what 16-way ILP hides on cached scattered global K/V reads. DO NOT propose cooperative LDS staging for K or V on flash_attn.",
      "GQA COLLAPSE STARVES R9700 WITHOUT SPLIT-K. Q_PER_KV=4 (-9.9%) and Q_PER_KV=2 (-17.5%) both lose standalone. Split-K (N_I_CHUNKS=4) restores WG count and won +4.1% (run-3 c12). If GQA is re-attempted, only combined with split-K so dispatch ≥ n_kv_heads × Q_PER_KV_BUDGET × n_i_chunks WGs.",
      "ATTENTION-SHADER MICRO-TUNING IS EXHAUSTED. V-load promotion (flat); drop redundant Phase-4 barriers (flat); subgroupShuffleXor merge (flat across 3 attempts); register-resident accumulators across blocks (flat); 32-way ILP (flat); narrow buffer-scoped barriers (flat); register-resident max_old/sum_old per-lane (run-4 c19, flat); subgroupBroadcast/Shuffle in split_merge (run-4 c29, flat); hoist Phase B reads ahead of Phase A in split_merge (run-4 c30, flat); pack 2 heads per WG cluster-32 (run-4 c14, flat); wave64 pinning on pipeline_q4k (run-4 c31, flat). The 16-way ILP + cluster-4/2 reductions are the saturation point.",
      "REMOVING gl_NumSubgroups>1 CROSS-SUBGROUP REDUCTION IN flash_attn REGRESSES. Run-4 cycle 32 removed the dead-looking branch in Phase 1 max + Phase 3 sum: -0.44 tok/s. The branch appears unused on wave64=1-subgroup but contributes to compiler scheduling decisions. Don't remove it.",
      "BLOCK_SIZE / N_I_CHUNKS TUNING IS FLAT. 256→512 (flat); 256→384 (+0.34 below threshold); N_I_CHUNKS=8 (flat vs 4). N_I_CHUNKS=4 is the sweet spot.",
      ...
      "WIDE NUM_ROWS DMMV VARIANTS UNDER-SATURATE. NUM_ROWS=8 dense Q4_K (-3.83); NUM_ROWS=8 Q6_K LM head (run-4 c25, flat — LM head fires once, can't move metric anyway). NUM_ROWS=2 is the right shape on dense Q4_K paths.",
      "TPB=32 DENSE Q4_K DMMV IS DEAD ACROSS 3 VARIANTS. Run-4 c5/c6 (-1.3%, -6.5% the un-installed and properly-installed variants); run-4 c33 (TPB=32 + cycle-22 loop swap, -0.83%). TPB=16 + NUM_ROWS=2 + the cycle-22 row/block loop swap is the right shape; don't try wider TPB on dense Q4_K.",
      "SPLIT-K / K-AXIS SPLIT ON DENSE Q4_K down_proj IS FLAT-TO-REGRESSING. Split-K kpar32_split + merge-add (run-4 c4, flat); K-axis N_K_CHUNKS=2 (-0.83%); N_K_CHUNKS=4 (-0.97%). The DMMV is already 78-85% of the bandwidth floor; further splitting adds merge-pass overhead that exceeds the parallelism gain.",
      "GATE+UP+SwiGLU SHADER MICRO-TUNES ARE FLAT. uvec4 alias for header reads (run-4 c8, flat); algebraic refactor `dot(f*q-b, by) = f*dot(q,by) - b*by_sum` with by_sum reuse (run-4 c13, -0.4%); 2-way paired accumulators (sg_a/sg_b, su_a/su_b) to break FMA dep chain (run-4 c16, flat); row/block loop swap mirroring cycle-22's pattern (run-4 c24, flat — gate+up already shares input reads via the existing fusion, no amortization to capture). The shader is at a local optimum; only structural changes (split-K on K=4096 axis) can help further.",
      "rms_norm FOLDED INTO ADJACENT MATVECS IS AT-OR-BELOW THRESHOLD. -0.37 (rms+gate+up+SwiGLU); +0.49 (rms+Q/K/V); +0.16 (ffn_norm into existing fusion). rms_norm is too small to measurably move the metric when fused. Don't re-attempt.",
      "rms_norm_mul Pass-2 PER-THREAD rms_inv IS FLAT. Run-4 c26: removing s_rms_inv LDS round-trip + 1 barrier per dispatch measured -0.12 (within noise). The barrier is amortized across the rest of the dispatch.",
      "K+V PROJECTION FUSION REGRESSES (-1.1%). Don't re-attempt.",
      "GATE+UP-ONLY FUSION (without SwiGLU) IS FLAT. Only wins WITH SwiGLU folded in.",
      "PER-TOKEN COS/SIN PRECOMPUTE FOR RoPE IS BELOW THRESHOLD (+0.29 vs needed +0.94).",
      "O_PROJ + ffn_norm CROSS-LAYER FUSION HAS THREE FAILED PATTERNS NOW. Separate-buffer (run-3 c5, -7%); atomic-counter cross-WG sync (run-3 c15, broken output); dmmv_q4k_o_proj_merge.comp via flash_attn_split_merge piggyback (run-4 c7, -67%; run-4 c28, -69% from LDS occupancy collapse). The dmmv_q4k_o_proj_merge.comp shader file is in tree but should be DELETED — it cannot be made to win in any wired form. The only untried pattern is last-WG-does-norm via gl_WorkGroupID.x==gl_NumWorkGroups.x-1 + sentinel-value-wait inside the last WG (no atomics, no separate buffer, no merge piggyback).",
      "sigmoid_mul (attn_gate) FOLDED INTO SPLIT-K MERGE PASS IS FLAT (mean +0.15, median -0.20).",
      "MANUAL-CYCLE + LANDED-LOOP FOUNDATIONS — DON'T REVERT. Q-stage (6ece0a8); s_kv_base_v4 precompute (run-1 c8 superseding f68b7d7); Phase 4 rescale+V-acc fusion (539b2aa); gate+up+SwiGLU dense FFN fusion (run-2 c8); Q+K norm+rope+kv_cache_write fusion (run-2 c12); split-K flash attention N_I_CHUNKS=4 + merge pass (run-3 c12); v_im uniform shift-scale Q4_K decode (run-4 c1); cluster-4 M/L reduction in split_merge (run-4 c22); cluster-2 chunk-pair split in split_merge (run-4 c23). All bit-correct.",
      "L=2325 GPU HANG IS L≥2300-ONLY. Confirmed across 80+ cycles. Watchdog-duration issue, not correctness.",
    ],
    structuralSwingIdeas: [
      "GATE+UP+SwiGLU SPLIT-K ON K=4096 AXIS — THE TOP TARGET. Run-4 cycle 12 revealed gate+up+SwiGLU is 60% of FFN (35% of total decode). All direct shader micro-tunes (algebraic, paired acc, loop swap, uvec4 alias) failed; the only structural shape untried is split-K. Mirror run-3 cycle 12's flash_attn split-K trick: split the K=4096 input axis into N_K_CHUNKS=2 or 4, dispatch n_chunks × n_M_tiles WGs with a merge pass that sums chunk partials into the SwiGLU output. The fused shader currently has NUM_ROWS=2 × M_tiles WGs; split-K multiplies that by N_K_CHUNKS for healthier SIMD occupancy. Risk: the gate/up shared-input-read (the cycle-8 fusion's main mechanism) is broken if K is partitioned across WGs — each chunk-WG re-reads its own slice of input. Mitigation: stage the input slice cooperatively in LDS (small, 4 KB at K=4096/N_K_CHUNKS=2) so within a WG gate and up still share the read. Multi-cycle.",
      "LAST-WG-DOES-NORM CROSS-LAYER FUSION. The only o_proj+ffn_norm pattern not yet disproved. Three failed predecessors: (a) separate-buffer indirection (run-3 c5, -7%); (b) atomic-counter cross-WG sync (run-3 c15, broken output); (c) merge-shader piggyback (run-4 c7/c28, -67%/-69% LDS occupancy collapse). The unattempted pattern: gl_WorkGroupID.x == gl_NumWorkGroups.x-1 detects last WG, which then spin-waits on a sentinel value other WGs write at end-of-acc, then runs rms_norm in-place on hidden_buf. No atomics, no separate buffer, no merge piggyback. Bit-correctness via memory_scope_semantics + memoryBarrierBuffer + sentinel-poll is the hard part. Build behind ZINC_LASTWG_OPROJ_NORM=1, validate with --validate-coherence at L=5/846, --validate-coherence on all COHERENCE_MODELS.",
      "MORE flash_attn_split_merge MICRO-WINS. Run-4 c22 (cluster-4 M/L reduction) and c23 (cluster-2 chunk-pair) both landed +0.5% individually. The merge shader still has surface area: per-lane partial-output vec4 packing (run-4 c30 hoist failed but the underlying packing question remains), 2-way unrolled chunk merge for N_I_CHUNKS=8 (untried with the cluster-2 win in place), bias-fold opportunities for models with attn biases. Cheap cycle-sized.",
      "flash_attn_cm1.comp KHR COOPMAT PORT. The structural ceiling for further attention speedup. R9700 advertises VK_KHR_cooperative_matrix. Multi-week port. Reference: /Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/flash_attn_cm1.comp. Pursue when the FFN attacks above are exhausted or shown structurally limited.",
      "INVESTIGATE down_proj's 2.4× BANDWIDTH-FLOOR GAP FROM A DIFFERENT ANGLE. Cycle 10's audit said down_proj is at 2.4× the Q4_K bandwidth floor (2.40 ms vs ~1 ms theoretical). Split-K and K-axis split both failed to recover this gap. Cycle 12's nextIdeas suggested hidden-dim-rotated dispatch order to break L1/L2 thrash patterns: instead of dispatching M tiles in 0..M_tiles-1 order, rotate by hidden-dim cache-line stride so consecutive WG launches don't compete for the same L2 lines. Diagnostic-style: cheap cycle, high uncertainty.",
      "GATE+UP+SwiGLU INPUT BROADCAST-VIA-LDS. The current fused shader has NUM_ROWS=2 × M_tiles WGs all reading the same input vector independently. Stage the K=4096 input vector into shared memory once per WG-group (e.g., dispatch n_groups × M_per_group WGs and have lane 0..255 of each WG cooperatively load 4 KB of input into LDS). Distinct from cycle-13's algebraic refactor (which targeted by_sum reuse) and cycle-24's loop swap (which only reordered loops). Risk: bandwidth bottleneck might already be captured by L1 cache; analogous staging on flash_attn K/V failed. Land behind ZINC_GATEUP_LDS_INPUT=1.",
    ],
    referenceImplementations: [
      {
