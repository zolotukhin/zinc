...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  6: {
    ...
    knownFlatCategories: [
      "GEMV-style cross-token MoE batching via dmmv_q4k_moe_batched.comp (commit c36bd23). Spent 9 wire-in cycles (cycles 1-9 of effort 6 run-3) producing flat or negative results. Architecturally wrong for RDNA4: dispatches 1.7M workgroups (M × n_experts_used × n_tokens) where the device caps at ~1024 in flight. The right approach is tiled GEMM with MUL_MAT_ID (see structuralSwingIdeas item 1), NOT more wire-in attempts at the GEMV pipeline. Pipeline + helper stay in tree as record but should not be wired.",
      "Three-way RMS+K+V or RMS+Q+K+V fusion shaders (cycles 11 of effort 6, cycle 14 of effort 10). Register pressure collapses occupancy on RDNA4; even with shared-memory RMS reduction, the per-WG VGPR budget runs out. Two-way RMS+single-output fusions (cycles 8, 13 of effort 10) are fine — three-way isn't.",
      "Triple-fused MoE swiglu+down+weighted-acc shader (cycle 9 of effort 6). Regressed -0.5%. The existing path's down + moe_weighted_acc dispatches already overlap on RDNA4; fusing them adds register pressure without saving wall time.",
      "Wide NUM_ROWS variants on dmmv_q4k_moe_kpar (cycle 12 of effort 10). NUM_ROWS=4 underutilizes occupancy at MoE expert M=1408 — the WG count drops below the 2-wave threshold for the device. Don't re-attempt wide NUM_ROWS variants on M ≤ ~10000 dispatches.",
      "Narrowing a single compute→compute barrier to buffer-scoped on RADV (cycles 1, 16, 18 of effort 10; multiple cycles of effort 6 first/second run). Cumulative movement below 0.25 tok/s. RADV's PipelineBarrier1 path doesn't differentiate the access masks the way the hypothesis assumed. Don't re-attempt without first proving via VkCmdPipelineBarrier2's explicit access masks that the API switch changes RADV's behavior.",
      ...
      "Pair-dispatch via recordBatchDispatch(num_cols=2) through the existing dmmv_q8_0_batch / dmmv_q4k_batch shaders. Cycle 8 of the second run measured -0.12 tok/s; cycle 9 rewrote the shader with proper wave64 parallelism and STILL measured -0.8 tok/s. The right fix is the tiled GEMM (item 1 in structuralSwingIdeas) when N > 8, NOT specialized num_cols=2 variants.",
    ...
    structuralSwingIdeas: [
      "PORT TILED Q4_K GEMM (no MoE yet) FROM llama.cpp. Reference: ~/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp. Use the #ifndef COOPMAT (warp-tiled MAC) branch since RDNA4 has no cooperative_matrix. Tile params to start: BM=64, BN=64, BK=32, WM=64, WN=32, WMITER=2, WNITER=2, TM=4, TN=2, local_size_x=128 (subgroup_size=64 × NUM_WARPS=2). Output: src/shaders/mul_mm_q4k.comp + pipeline registration in src/compute/dmmv.zig. Reuse the existing dmmv_q4k_moe_kpar.comp Q4_K block decode (lines 100-160) — bit layout matches llama.cpp's mul_mmq_funcs.glsl Q4_K branch (lines 303-364). Test against per-token Q4_K DMMV on a synthetic [M, K, N] matmul before integrating. This is the foundation step — no MoE yet; helps dense Gemma 4 31B prefill first.",
      "ADD MUL_MAT_ID VARIANT to the tiled GEMM (the MoE gather). Reference: ~/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_id_funcs.glsl — load_row_ids function uses subgroupBallot + subgroupBallotBitCount to gather tokens whose routed expert matches the WG's expert_idx into a row_ids[BN] shared list. The early-exit `if (ic*BN >= data_expert_count[expert_idx])` skips empty WGs. Most of the GEMM body is unchanged; just add #ifdef MUL_MAT_ID guards. Output: same shader file as item 1 with macro-driven variant. Reuses the existing routing_capture_buf (forward.zig:885) as data_ids.",
      "PORT count_experts.comp — trivial 50-line helper. Reference: ~/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/count_experts.comp. Reads the per-token expert_id buffer, outputs a [n_experts] count buffer. The MUL_MAT_ID GEMM uses this for early-exit. Port directly, no modifications needed.",
      "ADD mul_mmq_q4k.comp (Q8_1 activation variant of the GEMM). Reference: ~/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mmq.comp + mul_mmq_funcs.glsl Q4_K branch (lines 303-364). Same tiled GEMM as item 1 but inner dot uses dotPacked4x8EXT (GL_EXT_integer_dot_product). R9700 supports this — vulkaninfo confirms integerDotProduct4x8BitPackedSignedAccelerated=true. Adds another ~2× on top of items 1-2 because activation bandwidth drops 4× (f32 → Q8_1) and the dot is hardware integer. Reuse our existing src/shaders/quantize_q8_1.comp for the activation pre-quant.",
      "WIRE PIPELINE SELECTION in prefillBatched. After items 1-4 land: dense FFN uses mul_mm_q4k; MoE FFN uses mul_mm_id_q4k with data_ids from routing_capture_buf and data_expert_count from item 3; threshold to switch from per-token GEMV to GEMM at n_tokens > 8 (matches llama.cpp ggml-vulkan.cpp line 267 mul_mat_vec_max_cols=8). Relax canUseBatchedPrefillRdna's n_experts > 0 gate when pipeline_mul_mm_id_q4k is loaded. Validate ZINC_BATCHED_PREFILL=validate with tol=1e-2 for the mmq path (Q8_1 noise floor is ~2^-7) and tol=1e-3 for the plain f32 mul_mm path.",
    ],
    referenceImplementations: [
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp",
        focus: "FOUNDATION: tiled f16/f32 GEMM on Vulkan. Read the #ifndef COOPMAT branch (warp-tiled MACs) — RDNA4 has no cooperative_matrix so this is the path we port. Tile shape: BM=64 BN=64 BK=32, WM=64 WN=32, WMITER=2 WNITER=2, TM=4 TN=2, local_size_x=128 (subgroup_size=64 × NUM_WARPS=2). Dispatch: gridX = blocks_m × split_k, gridY = ceil(N/BN) tiles, gridZ = expert_idx (1 for dense). Output: src/shaders/mul_mm_q4k.comp + pipeline registration in src/compute/dmmv.zig. Step 1 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_funcs.glsl",
        focus: "Block-decode helpers used by mul_mm.comp. Read get_offsets() and DEQUANT_FUNC. Our existing src/shaders/dmmv_q4k_moe_kpar.comp already has a Q4_K block decoder (lines 100-160) whose bit layout matches — reuse it inside mul_mm_q4k.comp rather than re-deriving. Step 1 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_id_funcs.glsl",
        focus: "MUL_MAT_ID GATHER: this is the missing piece our 11 GEMV cycles never had. load_row_ids() uses subgroupBallot + subgroupBallotBitCount + subgroupBallotExclusiveBitCount to gather tokens whose routed expert == WG's expert_idx into a shared row_ids[BN] list of (token_idx, expert_slot) pairs. Early-exit: `if (ic*BN >= data_expert_count[expert_idx])`. Add as #ifdef MUL_MAT_ID variant of the same shader from step 1. Step 2 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/count_experts.comp",
        focus: "Trivial 50-line helper. Reads per-token expert_id buffer, atomically writes a [n_experts] count buffer. Used by MUL_MAT_ID GEMM for the early-exit above. Port directly, no modifications. Source: routing_capture_buf already populated by forward.zig:885. Step 3 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mmq.comp",
        focus: "Q8_1-activation variant of mul_mm.comp. Same tiled GEMM, but the inner-loop dot is dotPacked4x8EXT (GL_EXT_integer_dot_product). R9700 supports this: vulkaninfo confirms integerDotProduct4x8BitPackedSignedAccelerated=true. Activation bandwidth drops 4× (f32 → Q8_1) and the dot is hardware integer — adds another ~2× on top of steps 1-2. Output: src/shaders/mul_mmq_q4k.comp. Reuse our existing src/shaders/quantize_q8_1.comp for activation pre-quant. Step 4 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mmq_funcs.glsl",
        focus: "Q4_K decode for the mmq path. Lines 303-364 are the Q4_K stanza (and Q5_K right after). Slightly different from the f32 decode in step 1 because output is Q8_1 packed integers, not f32. Read alongside step 4. Step 4 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/ggml-vulkan.cpp",
        focus: "PIPELINE SELECTION threshold. Search for `mul_mat_vec_max_cols` (= 8): when N (token count) > 8, llama.cpp routes through mul_mm/mul_mm_id; when ≤ 8, through mul_mat_vec. Mirror this in src/compute/forward.zig prefillBatched: dense FFN → mul_mm_q4k, MoE FFN → mul_mm_id_q4k with data_ids=routing_capture_buf and data_expert_count from step 3. Relax canUseBatchedPrefillRdna's `n_experts > 0` gate once mul_mm_id_q4k is loaded. Validate with ZINC_BATCHED_PREFILL=validate, tol=1e-2 for mmq (Q8_1 noise floor ~2^-7), tol=1e-3 for f32 mul_mm. Step 5 of 5.",
      },
      {
        path: "/Users/stepan/Workspace/vllm/vllm/model_executor/layers/fused_moe",
        focus: "Cross-check reference for production MoE structure. Read fused_moe.py invoke_fused_moe_kernel + the Triton kernel: their gather is also (token_idx, expert_slot) per output row, confirming llama.cpp's design isn't backend-specific. Useful for understanding why per-expert WG dispatch wins over per-token GEMV at our scales. Not a port target — read for confidence only.",
      },
    ],
  },
};
