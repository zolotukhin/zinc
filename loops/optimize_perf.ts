...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  15: {
    ...
    knownFlatCategories: [
      ...
      "Do not round qwen36DensePrefillPaddedTokenCount up to 64 instead of 32 to dodge the DP4a gate-up ragged-n64 kernel variant. Tile-dispatch count is unchanged (same ceil(N/64)), but quantize_act_q8_1 + SwiGLU + fused-output cost scales with the padded N, so padding 212->256 instead of 212->224 is real extra elementwise work. Measured interleaved (baseline/fixed alternating in the same run, 5 rounds): baseline steady 227-230 tok/s, 64-pad steady 217-224 tok/s — a real ~3-6% regression, not the ~15% win a single non-interleaved sample suggested. Always interleave baseline/candidate runs on this node; a sequential before/after comparison is confounded by GPU clock drift.",
    ],
    structuralSwingIdeas: [
