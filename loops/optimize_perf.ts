...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  11: {
    ...
    knownFlatCategories: [
      "Q-stage in flash_attn.comp (commit 6ece0a8). Landed in manual cycle 71. Don't revert. The shared float s_q[512] cooperative load at the top is the right shape.",
      "Per-block page_ids[] cache (commit f68b7d7). Landed in manual cycle 72. Don't revert. Cycle 8 superseded with s_kv_base_v4 precompute that subsumes the page-id math entirely.",
      "Phase 4 rescale + V-acc fusion (commit 539b2aa). Bit-correct at L=1500 across 17 cycles of further build-on. Was suspected in the L=2325 GPU hang but the loop has measured at L=1500 for 18 cycles with no hangs, so the hang is L≥2300-only and likely a watchdog-duration issue, not a correctness bug. Don't revert; don't extend by piling more fused ops on top.",
      "Chunked V-tile LDS staging (cycle 2). CHUNK=16, s_v[2048]=8 KB LDS, replaced per-(d,i) global V read with cooperative load + LDS reads. Added ~16 barriers per block and apparent LDS bank pressure: -12% at L=1500. Reverted. Note: this kills the original Step-3 plan — V staging via this shape does NOT win.",
      "K-parallel Phase 1 with subgroupAdd reduction (cycle 1). Each i's Q.K dot computed cooperatively across 64 threads with subgroupAdd, giving coalesced K reads. Introduced per-i WG serialization at the reduction: -44% at L=846 vs baseline. Reverted. The 64-lane wave should NOT be ganged onto a single dot product.",
      "Subgroup shuffle merge for Phase 4 D-split (cycle 13). Replaced s_partial_v4 LDS round-trip with subgroupShuffleXor(acc, 32u) to merge upper/lower-half partial accumulators. Bit-correct, structurally cleaner (saved 512 B LDS + one barrier), but flat at the noise floor. Don't re-attempt unless the freed LDS is consumed by another cycle's structural change.",
      "Register-resident accumulators across blocks (cycle 15). Per-lane r_acc + per-lane local_max_old/local_sum_old + subgroupShuffleXor end-of-WG merge; eliminated per-block s_partial_v4/s_out_v4 LDS round-trip and two of three Phase-4 barriers. Bit-correct on Qwen 3 8B, performance flat. The LDS overhead of the round-trip is small at this scale; don't re-attempt unless paired with a different structural change that benefits from the freed LDS.",
      "32-way ILP unrolling in Phase 1 d4 dot-product (cycle 18). Six trials median 93.75 vs seven baseline trials median 93.77, t≈0.1, p≈0.9 — indistinguishable. The diminishing-returns curve on ILP unrolling has saturated at 16-way (cycles 16/17). Don't try 32-way or wider; VGPR pressure kills any further gain.",
    ],
    structuralSwingIdeas: [
      "MULTI-Q-PER-WG (GQA collapse, the highest-leverage remaining lever). Qwen 3 8B has q_per_kv = n_heads / n_kv_heads = 4. The current shader dispatches one workgroup per query head, so the 4 query heads sharing one KV head each load the SAME K/V cache rows independently. Process all 4 query heads in one WG: load K once into shared, do 4 Q.K dots, do 4 softmax states, accumulate 4 V-weighted outputs. Amortizes K/V tile reads 4-fold on attention-bound decode. Requires: (a) reduce dispatch grid.x from n_heads to n_kv_heads; (b) per-Q-head Q-stage now has 4 vec4 broadcasts; (c) 4 separate s_max_old/s_sum_old/s_out registers; (d) a single shared K row per inner iter dotted against 4 Q rows. Bandwidth utilization is currently ~260% effective vs theoretical (cycle 17 self-analysis), suggesting we're cache-amplified but each query head still pays its own K/V miss. Reference: llama.cpp flash_attn.comp Br=8 path + ggml-vulkan.cpp gqa_ratio collapse logic at ~line 8866. The agent has flagged this as 'next big lever' in cycles 8, 12, 14, 17.",
      "BLOCK_SIZE 256 → 512. Halves per-block iteration overhead at L=1500 (3 outer iters vs 6). Adds 2 KB to s_scores/s_kv_base_v4/s_partial_v4 (or whatever the equivalent allocations cost at 2x block) plus one extra V chunk's worth of register pressure on the 16-way ILP path. Risk: drops occupancy by one wave per SIMD if VGPR/LDS edges cross a threshold. A/B: build with BLOCK_SIZE=512 only, run L=1500 + L=5 + Qwen 3.6 35B coherence. If perf neutral or positive, ship; if negative, revert. Cycle-sized.",
      "DROP THE PHASE-4-MERGE-TO-SOFTMAX-STATE BARRIER. After the cycle-13 subgroup shuffle merge in Phase 4, the per-lane d4 ownership of s_out_v4 means no in-iter cross-lane race remains. The barrier between Phase 4 D-split merge and lane-0's softmax state update (`s_sum_old = s_sum_old * rescale + block_sum`) is therefore redundant. Cross-iter races are still covered by the trailing barrier and the s_kv_base_v4 fill barrier in step-1 of the next iter. Single-line removal; cycle 13's self-analysis flagged it as a clean follow-up.",
      "COOPERATIVE K-ROW STAGING (chunk=32, distinct from cycle 1's K-parallel and cycle 2's V-staging). Cycle 1's failure was using subgroupAdd to reduce within one row; this proposal stages K cooperatively into LDS once per block and has each thread broadcast-read its own K row from LDS at ~10-cycle latency vs L1's 50-100 cycles. Differs from V staging because K is read in Phase 1 only (not the inner Phase 4 loop), so the LDS write→read amortizes over fewer iterations and the bank-pressure pattern is different. CHUNK=32 fits in 16 KB. Land behind ZINC_FA_K_TILE=1; promote on perf-positive at L=1500.",
      "PROMOTE V-LOAD ABOVE THE SCORE MULTIPLY in Phase 4 D-split inner loop. Currently each iter does (score-load-from-s_scores) → (V-load-from-v_data) → (multiply-add). Re-ordering to (V-load) → (score-load) → (multiply-add) lets the next-iter V-load issue while the current iter's FMA pipeline is still draining, exposing one more level of load-load parallelism. Cycle 17's self-analysis flagged it.",
      "PHASE 3+4 REGISTER-RESIDENT COHESION. Keep Pf[r] = exp(score - max) in registers across the V-acc loop instead of round-tripping through s_scores[]. Cycle 15 attempted a related variant (per-lane r_acc, persistent) and measured flat. This proposal is narrower: keep ONLY Pf[r] in registers within one block iter, not across blocks. Less invasive than cycle 15's design.",
      "POPULATE flash_attn_cm1.comp COOPMAT VARIANT. RDNA3+ supports VK_KHR_cooperative_matrix and R9700 advertises it. Cooperative-matrix wmma intrinsics on the Q.K matmul and the softmax-V matmul could deliver substantial throughput on attention if the layout matches. Multi-week port; price separately. Reference: /Users/stepan/Workspace/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/flash_attn_cm1.comp. Pursue only after the cheaper structural wins above are exhausted.",
    ],
    referenceImplementations: [
      {
