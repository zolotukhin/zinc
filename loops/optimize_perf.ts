...
const EFFORT_SPECS: Record<number, EffortSpec> = {
  ...
  6: {
    ...
    structuralSwingIdeas: [
      "Wire in cross-token batched MoE FFN. The shader (src/shaders/dmmv_q4k_moe_batched.comp) and dispatch helper (DmmvDispatch.recordMoeBatchedDispatch) shipped in commit c36bd23 — pipeline loads at startup, no call site uses it. This is the SINGLE biggest lever for closing the 40-41% prefill gap to llama.cpp on Qwen 3.5/3.6 35B. Wire-in steps: (1) batched router DMMV for all N tokens via existing dmmv_q4k_batch_kpar shader; (2) loop softmax-topk per token; (3) write routing into existing routing_capture_buf (forward.zig:885); (4) allocate [N × n_experts_used × inter_dim] output scratch; (5) dispatch batched gate/up/down via recordMoeBatchedDispatch; (6) write a moe_weighted_acc_batched.comp shader that takes [N × n_experts_used × inter] expert outputs + routing weights and writes [N × hidden] back into scratch_hidden; (7) relax canUseBatchedPrefillRdna's n_experts > 0 gate when pipeline_q4k_moe_batched is loaded; (8) on layers that are SSM (qwen35moe full_attn_interval=4 → 30 of 40 SSM), fall back to per-token within prefillBatched body until parallel-scan SSM lands. Validate via ZINC_BATCHED_PREFILL=validate.",
