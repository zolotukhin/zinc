...
- [ ] T001 Add server mode to main — when no --prompt flag, start HTTP server on configured port. Import and call Server.init + accept loop. File: src/main.zig
- [ ] T002 Implement full HTTP/1.1 request parsing — read headers until \r\n\r\n, extract method, path, Content-Length, read body. Replace current stub readRequest. File: src/server/http.zig
- [ ] T003 [P] Add HTTP response helpers — sendJson (status + JSON body), sendError (OpenAI error format), sendSseStart (streaming headers). File: src/server/http.zig
...
- [ ] T004 Implement SSE stream writer — chunked transfer encoding, write `data: {json}\n\n` events, `data: [DONE]\n\n`, detect client disconnect via write failure. File: src/server/sse.zig
- [ ] T005 [P] Implement route dispatcher — match method+path to handler functions. Routes: POST /v1/chat/completions, POST /v1/completions, GET /v1/models, GET /health. Return 404 for unknown routes. File: src/server/routes.zig
...
- [ ] T009 [US1] Implement POST /v1/chat/completions handler — parse JSON request body (model, messages, stream, max_tokens, temperature, stop), validate required fields, return 400 on errors. Apply chat template to messages, tokenize prompt. File: src/server/routes.zig
- [ ] T010 [US1] Implement streaming response path — for stream:true, create SSE writer, submit request to scheduler, run decode loop sending ChatCompletionChunk events per token, send [DONE] on completion. Include id, object, created, model, choices[0].delta fields. File: src/server/routes.zig
- [ ] T011 [US1] Implement non-streaming response path — for stream:false or omitted, run full generation, build ChatCompletion response with complete message, usage (prompt_tokens, completion_tokens, total_tokens), finish_reason. File: src/server/routes.zig
...
- [ ] T013 [US1] Generate unique request IDs — format "chatcmpl-{hex}" using timestamp + counter. Include in all response objects. File: src/server/routes.zig
...
- [ ] T019 [US3] Implement POST /v1/completions handler — parse prompt (string), apply same generation logic as chat completions but without chat template. Return text_completion objects. File: src/server/routes.zig
- [ ] T020 [US3] Implement GET /v1/models handler — return list containing the loaded model with id, object, created, owned_by fields. Model name derived from GGUF filename. File: src/server/routes.zig
...
- [ ] T024 [US4] Implement GET /health handler — return status ("ok" or "loading"), model name, active_requests count from scheduler, max_parallel, uptime_seconds. Return 503 during model loading. File: src/server/routes.zig
...
- [ ] T026 [P] Add CORS headers to all responses — Access-Control-Allow-Origin: *, required for browser-based clients. File: src/server/http.zig
